...
import { alpha, Box, Button, IconButton, Typography, useMediaQuery, useTheme } from '@mui/material';
...
export function SingleActivityRenderer({
  ...
  useEffect(() => {
    ...
    if (isOverview) {
      ...
      const x = (box.width / size) * (index % 3) + gapPx;
      const y = (box.height / size) * Math.floor(index / 3) + gapPx;
      const width = box.width / size - gapPx * (size - 2);
      const height = box.height / size - gapPx * (size - 2);
  ...
  return (
    <ActivityContext.Provider value={activity}>
      ...
      >
        ...
        >
          {!minimized && !isOverview && !isSmallScreen && (
            <ActivityDragger
                ...
                {!isSmallScreen && (
                  <>
                    {location === 'full' ? (
                      <IconButton
                        size="small"
                        onClick={() => Activity.update(id, { location: 'window' })}
                        title={t('Window')}
                      >
    ...
    >
      {activities.map(it => (
        ...
        >
          ...
          >
            <Box sx={{ width: '18px', height: '18px', flexShrink: 0 }}>{it.icon}</Box>
            <Box
              sx={{
                whiteSpace: 'nowrap',
                overflow: 'hidden',
      ...
      <Button
        ...
        startIcon={<Icon icon="mdi:grid-large" />}
      >
